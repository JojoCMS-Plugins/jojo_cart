...
<div id="fm_{$fd_field}_region"{if $freight_type!='region'} style="display:none"{/if}  class="fm_{$fd_field}_freight">
    ...
    <div class="tab-content">
        ...
        {foreach from=$methods key=methodid item=method}
                    <td><input style="text-align: right" type="text" size="6" name="fm_{$fd_field}_default_min[{$methodid}]" value="{$freight_default_min[$methodid]}" /><span class="help-inline">{$OPTIONS.cart_default_currency}</span></td>
        {/foreach}
                </tr>
        ...
        {foreach from=$methods key=methodid item=method}
                    <td><input style="text-align: right" type="text" size="6" name="fm_{$fd_field}_region_min_{$freight_regions[r].code}[{$methodid}]" value="{$freight_regions_min[r][price][$methodid]}" autocomplete="off" /><span class="help-inline">{$OPTIONS.cart_default_currency}</span></td>
        {/foreach}
                </tr>
        {/section}
            </table>
        ...
        {foreach from=$methods key=methodid item=method}
                    <td><input style="text-align: right" type="text" size="6" name="fm_{$fd_field}_default_base[{$methodid}]" value="{$freight_default_base[$methodid]}" /><span class="help-inline">{$OPTIONS.cart_default_currency}</span></td>
        {/foreach}
                </tr>
        {section name=r loop=$freight_regions}
                <tr>
                    <td>{$freight_regions[r].name}</td>
        {foreach from=$methods key=methodid item=method}
                    <td><input style="text-align: right" type="text" size="6" name="fm_{$fd_field}_region_base_{$freight_regions[r].code}[{$methodid}]" value="{$freight_regions_base[r][price][$methodid]}" autocomplete="off" /><span class="help-inline">{$OPTIONS.cart_default_currency}</span></td>
        {/foreach}
                </tr>
        {/section}
            </table>
        ...
        {foreach from=$methods key=methodid item=method}
                    <td><input style="text-align: right" type="text" size="6" name="fm_{$fd_field}_default[{$methodid}]" value="{$freight_default[$methodid]}" /><span class="help-inline">{$OPTIONS.cart_default_currency}</span></td>
        {/foreach}
                </tr>
        {section name=r loop=$freight_regions}
                <tr>
                    <td>{$freight_regions[r].name}</td>
        {foreach from=$methods key=methodid item=method}
                    <td><input style="text-align: right" type="text" size="6" name="fm_{$fd_field}_region_{$freight_regions[r].code}[{$methodid}]" value="{$freight_regions[r][price][$methodid]}" autocomplete="off" /><span class="help-inline">{$OPTIONS.cart_default_currency}</span></td>
        {/foreach}
                </tr>
        {/section}
            </table>
