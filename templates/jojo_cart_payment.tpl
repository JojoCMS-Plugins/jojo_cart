  ...
  {* Subtotal *}
   ...
   <div id="cart-freight">
    ...
    <div id="cart-total">
        ...
        {if $OPTIONS.cart_show_gst != 'no' && (($order.currency=='NZD') || ($order.currency=='' && $OPTIONS.cart_default_currency=='NZD'))}<p class="note">includes GST of {$order.currency_symbol|default:' '}{$order.amount/7.66666|string_format:"%01.2f"}{/if}</p>{/if}
    </div>
{if $usediscount}
